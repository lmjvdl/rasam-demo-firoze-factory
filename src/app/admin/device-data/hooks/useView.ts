...
export default function getDeviceDataList(pages: number, pageSize: number, URL: string | null) {
  const queryClient = useQueryClient();
  const { showToast } = useToast();

  const getDeviceDataListMutation = useMutation({
    mutationKey: allQueryKeys.adminPanel.deviceData.list,
    retry: false,
    mutationFn: ({ page = pages, page_size = pageSize, url = URL }: { page?: number; page_size?: number; url: string | null; }) =>
      fetchWithError(
        url !== null ? url : 
        `${deviceDataUrls.listDeviceData}?p=${page}&page_size=${page_size}`,
        { method: "GET" }
      ).then(sanitizer),
    onSuccess: (serverResponse) => {
      queryClient.setQueryData(allQueryKeys.adminPanel.deviceData.list, {
  ...
  return getDeviceDataListMutation;
}
