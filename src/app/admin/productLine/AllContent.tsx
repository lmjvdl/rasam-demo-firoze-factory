...
import useProductLineList, { ResponseSchema } from "./hooks/useView";
...
const AllContentProductLine: React.FC = () => {
  ...
  const getList = useProductLineList(pageNumber, 8, nextPage);
  ...
  const handleSaveEdit = (updatedRow: ProductLineUpdateSchema) => {
    setData((prevData) => {
      if (prevData?.data) {
        updateProductLineMutation.mutate(updatedRow);
...
export default AllContentProductLine;