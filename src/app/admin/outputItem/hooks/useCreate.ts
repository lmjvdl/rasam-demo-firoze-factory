...
export const createNewOutputItem = async (data: unknown) => {
  const validationResult = outputItemSchema.safeParse(data);
  ...
  try {
    const response = await fetchWithErrorForCreate(`${outputItemUrls.createOutputItem}`, {
      method: "POST",
